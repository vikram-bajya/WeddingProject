...
import { MapPin, Star, Heart, CheckCircle, Image as ImageIcon, Phone, MessageCircle } from 'lucide-react'
...
const VENDOR_DATA = {
  ...
  description: 'Specializing in candid wedding photography, Sharma Wedding Photography captures the true essence and raw emotions of your big day. With over 10 years of experience shooting luxury weddings across India, our team ensures every fleeting smile and tear of joy is immortalized in cinematic quality.',
  images: [
  ...
  packages: [
    {
      name: 'Basic Photo',
      price: '₹75,000',
      features: ['1 Candid Photographer', '1 Traditional Photographer', 'Edited High-Res Images', 'Soft Copy Delivery (Pendrive)'],
    },
    {
      name: 'Standard Combo',
      price: '₹1,25,000',
      features: ['2 Candid Photographers', '1 Traditional Photographer', '1 Cinematic Videographer', '3-5 Min Wedding Trailer', 'Coffee Table Photo Book'],
    },
    {
      name: 'Premium Cinematic',
      price: '₹2,00,000',
      features: ['Full Team (Photo & Video)', 'Drone Coverage', 'Pre-Wedding Shoot (1 Day)', 'Same Day Edit Video', '2 Premium Coffee Table Books'],
    }
  ]
}
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-10">
        <div className="relative rounded-2xl overflow-hidden mb-2" style={{ height: '500px' }}>
          <img src={VENDOR_DATA.images[0]} alt="Hero" className="w-full h-full object-cover" />
        ...
        <div className="flex gap-2 relative">
          {VENDOR_DATA.images.slice(1).map((img, i) => (
            <div key={i} className={`flex-1 h-32 rounded-xl overflow-hidden ${i === 3 ? 'relative cursor-pointer hover:opacity-90' : ''}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-7 xl:col-span-8">
            <div className="mb-6">
              ...
              <p className="text-gray-600 leading-relaxed">{VENDOR_DATA.description}</p>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                {VENDOR_DATA.packages.map((pkg, i) => (
                  <div key={i} className={`card p-6 border-2 ${i === 1 ? 'border-[#66D0BC] relative' : 'border-transparent'}`}>
                    ...
                    <button className="btn-outline w-full text-sm py-2">Book Package</button>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="sticky top-24">
              <div className="card p-6 border border-gray-100">
                ...
                <form className="space-y-4">
                  <div>
                    <input type="text" placeholder="Your Full Name" className="input-field py-2.5" />
                  </div>
                  <div>
                    <input type="email" placeholder="Email Address" className="input-field py-2.5" />
                  </div>
                  <div className="flex gap-2">
                    ...
                    <div className="w-2/3">
                      <input type="tel" placeholder="Phone Number" className="input-field py-2.5" />
                    </div>
                  </div>
                  <div>
                    <input type="date" value={date} onChange={e=>setDate(e.target.value)} className={`input-field py-2.5 ${!date ? 'text-gray-400' : 'text-gray-700'}`} />
                  </div>
                  <div>
                  ...
                  <div>
                    <textarea rows="3" placeholder="Tell them about your dream wedding..." className="input-field py-2.5 resize-none"></textarea>
                  </div>
                  <button type="submit" className="btn-primary w-full py-3">Send Enquiry</button>
                </form>
