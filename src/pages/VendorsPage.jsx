    ...
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <span
          key={i}
          style={{
            color: i <= Math.round(rating) ? "#66D0BC" : "#ddd",
            fontSize: "0.9rem",
          }}
        >
          ★
...
export default function VendorsPage() {
  ...
  const filtered = ALL_VENDORS.filter((v) => {
    const matchSearch =
      ...
      v.location.toLowerCase().includes(appliedFilters.search.toLowerCase());
    const matchCat =
      appliedFilters.category === "All" ||
      v.category === appliedFilters.category;
    const matchCity =
      appliedFilters.cities.length === 0 ||
      appliedFilters.cities.includes(v.location);
    const matchRating =
      !appliedFilters.rating ||
      (appliedFilters.rating === "5★ Only"
        ? v.rating >= 4.9
        : appliedFilters.rating === "4★ & above"
          ? v.rating >= 4
          : v.rating >= 3);
            ...
            <div className="card p-6 sticky top-36">
              ...
              <div className="mb-6 border-b border-gray-100 pb-4">
                <label className="flex items-center gap-2 cursor-pointer text-sm font-semibold text-[#FF3E9B]">
                  ...
                  <Heart
                    size={16}
                    className={showFavorites ? "fill-[#FF3E9B]" : ""}
                  />{" "}
                  Show Favorites Only
                </label>
              </div>
              ...
              <div className="mb-6">
                ...
                {CATEGORIES.map((cat) => (
                  <label
                    key={cat}
                    className="flex items-center gap-2 mb-2 cursor-pointer text-sm text-gray-600 hover:text-[#FF3E9B]"
                  >
                    <input
                      type="radio"
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-6">
              <p className="text-gray-600 text-sm">
                Showing <strong>{filtered.length}</strong> vendors
              </p>
              <select
                value={sort}
            ...
            {currentItems.length === 0 ? (
              <div className="text-center py-20 text-gray-400 bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <button
                  onClick={clearFilters}
                  className="mt-4 btn-outline py-2 px-6 text-sm"
                >
                  Clear Filters
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {currentItems.map((vendor) => (
                  <div
                    key={vendor.id}
                    className="card flex flex-col h-full hover:-translate-y-1 transition-transform duration-300"
                  >
                    ...
                    <div className="p-5 flex flex-col flex-grow">
                      ...
                      <div className="mt-auto pt-4 border-t border-gray-100">
                        ...
                        <Link
                          ...
                          className="btn-primary w-full text-center text-sm py-2.5 block rounded-md"
                        >
                          View Details
                        </Link>
                      </div>
