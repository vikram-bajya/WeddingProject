...
import { Menu, X, Heart, User, ChevronDown, Settings, PencilLine, LogOut } from 'lucide-react'

export default function Header() {
  const [menuOpen, setMenuOpen] = useState(false)
  const [profileOpen, setProfileOpen] = useState(false)
  const [isLoggedIn, setIsLoggedIn] = useState(false)
  const [userProfile, setUserProfile] = useState({ name: 'User', email: '', photo: '' })
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            {!isLoggedIn ? (
              ...
              <div className="relative" ref={profileMenuRef}>
                ...
                >
                  {userProfile?.photo ? (
                    <img src={userProfile.photo} alt={userProfile.name || 'User'} className="h-9 w-9 rounded-full object-cover border border-gray-100" />
                  ) : (
                ...
                {profileOpen && (
                  <div className="absolute right-0 mt-2 w-48 rounded-lg border border-gray-100 bg-white shadow-lg py-2 z-50">
                    <div className="px-4 pb-2 mb-1 border-b border-gray-100">
                      <p className="text-sm font-semibold text-gray-800 truncate">{userProfile?.name || 'User'}</p>
                      <p className="text-xs text-gray-500 truncate">{userProfile?.email || 'No email'}</p>
                    </div>
                    <Link to="/profile" className="flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-50" onClick={() => setProfileOpen(false)}>
                      <User size={16} />
                      Profile
                    </Link>
                    <Link to="/settings" className="flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-50" onClick={() => setProfileOpen(false)}>
                      <Settings size={16} />
                      Settings
                    </Link>
                    <Link to="/edit-profile" className="flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-50" onClick={() => setProfileOpen(false)}>
                      <PencilLine size={16} />
                      Edit Profile
                    </Link>
                    <button type="button" className="w-full text-left flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-50 text-red-500" onClick={handleLogout}>
                      <LogOut size={16} />
                      Log out
                    </button>
                  </div>
