...
const pool = new Pool({
  connectionString:
    ...
    "postgresql://postgres:postgres@localhost:5432/weddingbliss",
});
...
app.get(
  ...
  function (req, res) {
    ...
    res.redirect(
      `${redirectBase}/login?oauth=success&name=${name}&email=${email}&photo=${photo}`,
    );
  },
);
...
app.get("/api/vendors/:id", async (req, res) => {
  ...
  try {
    ...
    const imagesResult = await pool.query(
      "SELECT * FROM vendor_images WHERE vendor_id = $1",
      [id],
    );
